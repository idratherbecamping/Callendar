  ...
  useEffect(() => {
    async function handleCallback() {
      try {
        ...
        const tokenResponse = await fetch('/api/square/exchange-token', {
          method: 'POST',
          headers: {
        ...
        if (!tokenResponse.ok) {
          throw new Error('Failed to exchange authorization code for Square tokens')
        }
        
        const tokenData = await tokenResponse.json()
        
        // Update the form data to include Square tokens and connection status
        formData.squareConnected = true
        formData.squareAuthToken = {
      ...
      <div className="max-w-md w-full space-y-8 text-center">
        ...
        {status === 'error' && (
          <>
            ...
            <h2 className="mt-6 text-center text-xl font-medium text-gray-900">
              Failed to connect Square account
            </h2>
